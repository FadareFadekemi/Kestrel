...
import { initiateJsProPayment, verifyJsProPayment } from '../../services/paymentApi';
...
export default function PaywallModal({ feature, user, onClose, onUpgradeSuccess }) {
  const { isDark, colors: c } = useTheme();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState(false);

  const featureInfo = FEATURE_MAP[feature] || { label: feature, icon: Lock };
  ...
  async function handleUpgrade() {
    ...
    setError('');
    try {
      const { reference, public_key, email, amount } = await initiateJsProPayment();
      const handler = window.PaystackPop.setup({
        ...
        onSuccess: async (tx) => {
          setLoading(true);
          try {
            const result = await verifyJsProPayment(tx.reference);
            if (result.plan === 'pro') {
              setSuccess(true);
              onUpgradeSuccess?.();
            } else {
              setError('Payment received but upgrade is pending. Please refresh in a moment.');
            }
          } catch {
            setError('Verification failed. Contact support if you were charged.');
          } finally {
            setLoading(false);
          }
        },
  ...
  if (success) {
    return (
      <div style={{
        ...
        background: overlayBg, padding: 20,
      }}>
        <div style={{
          background: modalBg, borderRadius: 20, padding: '48px 40px',
  ...
  return (
    ...
    >
      <div style={{
        background: modalBg, borderRadius: 20, padding: '36px 32px',
        ...
        <button onClick={onClose} style={{
          ...
          background: 'var(--card-2)', border: 'none', borderRadius: 8,
        ...
        {error && (
          <p style={{ color: 'var(--error)', fontSize: 13, textAlign: 'center', margin: '12px 0 0' }}>
            {error}
          </p>
        )}
