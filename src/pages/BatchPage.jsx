  ...
  return (
    <div style={{ padding: '28px 32px', overflowY: 'auto', height: '100%' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: c.txt, margin: 0 }}>Batch Mode</h1>
        <p style={{ fontSize: 13, color: c.mut, marginTop: 4 }}>Process multiple leads simultaneously</p>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        >
          <div style={{ width: 52, height: 52, background: c.ad, border: '1px solid rgba(0,212,200,0.15)', borderRadius: 12, margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Upload size={22} color="#00D4C8" />
          </div>
          <p style={{ fontSize: 16, fontWeight: 600, color: c.txt, margin: '0 0 6px' }}>Drop your CSV here</p>
          <p style={{ fontSize: 13, color: c.mut, margin: 0 }}>or click to browse. Columns: <code style={{ background: c.brd, padding: '1px 5px', borderRadius: 4, fontSize: 11 }}>company, url</code></p>
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <div style={{ flex: 1, background: c.card, border: `1px solid ${c.brd}`, borderRadius: 8, padding: '10px 14px', display: 'flex', align: 'center', gap: 10 }}>
              <FileText size={14} color="#00D4C8" />
              <span style={{ fontSize: 13, color: c.txt }}>{rows.length} leads loaded</span>
            </div>
            {running && (
              <div style={{ display: 'flex', align: 'center', gap: 8, background: c.card, border: `1px solid ${c.brd}`, borderRadius: 8, padding: '10px 14px' }}>
                <span style={{ fontSize: 13, color: c.mut }}>{totalDone}/{rows.length} processed</span>
              </div>
            )}
            <button onClick={() => { setRows([]); setResults([]); setProgress({}); }} disabled={running}
              style={{ background: c.brd, border: '1px solid #264040', borderRadius: 8, padding: '9px 14px', fontSize: 12, color: c.mut, cursor: running ? 'not-allowed' : 'pointer' }}>
              Clear
            </button>
            {results.filter(r => r?.lead).length > 0 && (
              <button onClick={() => exportCSV(results)} style={{ background: c.brd, border: '1px solid #264040', borderRadius: 8, padding: '9px 14px', fontSize: 12, color: c.txt, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Download size={13} /> Export CSV
              </button>
            )}
            <button onClick={runBatch} disabled={running}
              style={{ background: running ? c.brd : '#00D4C8', color: running ? c.mut : c.bg, border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 600, cursor: running ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {/* Progress bars */}
          {running && (
            <div style={{ marginBottom: 20, background: c.card, border: `1px solid ${c.brd}`, borderRadius: 10, padding: '12px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontSize: 12, color: c.mut }}>Pipeline Progress</span>
              ...
              <thead>
                <tr style={{ borderBottom: `1px solid ${c.brd}` }}>
                  {['#', 'Company', 'Score', 'Industry', 'Tech Stack', 'Email', 'Status'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '10px 16px', fontSize: 11, fontWeight: 600, color: c.mut, letterSpacing: '0.5px' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: i < rows.length - 1 ? '1px solid #1c1c1e' : 'none' }}>
                      <td style={{ padding: '10px 16px', fontSize: 12, color: c.mut }}>{i + 1}</td>
                      <td style={{ padding: '10px 16px' }}>
                        <div>
                          <p style={{ fontSize: 13, fontWeight: 500, color: c.txt, margin: 0 }}>{lead?.company || row.company || row.url}</p>
                          {(row.url && !lead) && <p style={{ fontSize: 10, color: c.mut, margin: 0 }}>{row.url}</p>}
                        </div>
                      </td>
                      <td style={{ padding: '10px 16px' }}>
                        {lead ? <ScoreRing score={lead.score} size={36} /> : <span style={{ fontSize: 12, color: '#264040' }}>—</span>}
                      </td>
                      <td style={{ padding: '10px 16px', fontSize: 12, color: c.mut }}>{lead?.industry || '—'}</td>
                      <td style={{ padding: '10px 16px' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                          {lead?.techStack?.slice(0, 3).map(t => (
                            <span key={t} style={{ fontSize: 10, color: '#00D4C8', background: c.ad, border: '1px solid rgba(0,212,200,0.15)', borderRadius: 4, padding: '1px 5px' }}>{t}</span>
                          )) || <span style={{ fontSize: 12, color: '#264040' }}>—</span>}
                        </div>
                      </td>
                      <td style={{ padding: '10px 16px', fontSize: 12, color: c.mut, maxWidth: 200 }}>
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        ...
                        {p?.status === 'pending' && <span style={{ fontSize: 11, color: c.mut }}>Queued</span>}
                      </td>
                    </tr>
                  );
                })}
