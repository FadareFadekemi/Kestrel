...
function ScoreRing({ score }) {
  ...
  return (
    <svg width={40} height={40} style={{ flexShrink: 0 }}>
      <circle cx={cx} cy={cx} r={r} fill="none" stroke={themeColors.brd} strokeWidth={4} />
      <circle cx={cx} cy={cx} r={r} fill="none" stroke={color} strokeWidth={4}
        strokeDasharray={`${dash} ${circ}`} strokeLinecap="round"
        transform={`rotate(-90 ${cx} ${cx})`} />
      <text x={cx} y={cx + 4} textAnchor="middle" fill={color} fontSize={10} fontWeight={700}>{score}</text>
    </svg>
  );
}
...
function CompanyCard({ company, onResearch }) {
  ...
  const color = sizeColors[company.size] || c.mut;
  return (
    <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 12, padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
            <p style={{ fontSize: 14, fontWeight: 700, color: c.txt, margin: 0 }}>{company.name}</p>
            {company.verified && (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 11, color: c.mut, display: 'flex', alignItems: 'center', gap: 4 }}><Building2 size={11} /> {company.sector}</span>
            <span style={{ fontSize: 11, color: c.mut, display: 'flex', alignItems: 'center', gap: 4 }}><MapPin size={11} /> {company.location}</span>
      ...
      <p style={{ fontSize: 12, color: c.mut2, margin: 0, lineHeight: 1.6 }}>{company.description}</p>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
        ...
        {company.website && (
          <button onClick={() => window.open(`https://${company.website}`, '_blank', 'noopener')}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: 'rgba(161,161,170,0.08)', border: '1px solid #264040', borderRadius: 8, padding: '8px 12px', color: c.mut, fontSize: 12, cursor: 'pointer' }}>
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px' : '28px 32px', overflowY: 'auto', height: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: c.txt, margin: 0 }}>Job Matches</h1>
      ...
      {loading && (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 16, maxWidth: 900, marginBottom: 24 }}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 12, padding: 18 }}>
      ...
      {!loading && companies.length > 0 && (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
          <select value={filterSize} onChange={e => setFilterSize(e.target.value)}
            style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 7, padding: '6px 10px', color: '#C5E8E6', fontSize: 12, cursor: 'pointer', outline: 'none' }}>
          ...
          <button onClick={() => setRemoteOnly(v => !v)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, background: remoteOnly ? 'rgba(96,165,250,0.12)' : c.card, border: `1px solid ${remoteOnly ? 'rgba(96,165,250,0.3)' : c.brd}`, borderRadius: 7, padding: '6px 10px', color: remoteOnly ? '#60a5fa' : c.mut, fontSize: 12, cursor: 'pointer' }}>
            {remoteOnly ? <ToggleRight size={14} /> : <ToggleLeft size={14} />} Remote only
          </button>

          <span style={{ fontSize: 12, color: c.mut, marginLeft: 4 }}>
      ...
      {!loading && companies.length > 0 && filtered.length === 0 && (
        <div style={{ background: c.card, border: '1px dashed #264040', borderRadius: 12, padding: '40px 20px', textAlign: 'center', maxWidth: 480, marginBottom: 32 }}>
          <Building2 size={28} color="#264040" style={{ marginBottom: 12 }} />
          <p style={{ fontSize: 13, color: c.mut, margin: 0 }}>No companies match the current filters. Try adjusting the size or sector.</p>
        </div>
      )}

      {/* Pre-search empty state */}
      {!loading && companies.length === 0 && !error && hasProfile && (
        <div style={{ background: c.card, border: '1px dashed #264040', borderRadius: 12, padding: '48px 24px', textAlign: 'center', maxWidth: 480, marginBottom: 32 }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: c.ad, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <Briefcase size={22} color="#00D4C8" />
          </div>
          <p style={{ fontSize: 14, fontWeight: 600, color: c.txt, margin: '0 0 8px' }}>Ready to find your matches</p>
          <p style={{ fontSize: 13, color: c.mut, margin: '0 0 20px', lineHeight: 1.6 }}>
        ...
        <div style={{ maxWidth: 900, background: 'rgba(52,211,153,0.04)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: 10, padding: '14px 18px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#6b7280', margin: 0, lineHeight: 1.5 }}>
              Companies shown are AI-researched. Always verify independently before sharing personal details. Never pay for a job application. Use the <strong style={{ color: c.mut2 }}>Scam Detector</strong> if you receive an unsolicited offer.
            </p>
          </div>
        </div>
      )}
