...
const Pill = ({ children, color }) => {
  ...
  const clr = color ?? c.a;
  return (
    <span style={{ fontSize: 11, fontWeight: 700, color: clr, background: `${clr}18`,
      border: `1px solid ${clr}30`, borderRadius: 20, padding: '3px 10px',
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 520 }}>
      ...
      <div className="animate-glow" style={{
        ...
        background: `radial-gradient(ellipse at 30% 50%, ${c.ad} 0%, transparent 70%)`,
        pointerEvents: 'none',
      }} />
      ...
      <div className="animate-float" style={{
        ...
        <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 14,
          padding: 16, borderTop: `2px solid ${c.a}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
            <Search size={12} color={c.a} />
            <span style={{ fontSize: 11, fontWeight: 700, color: c.a, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Company Intel</span>
          </div>
          <p style={{ fontSize: 13, fontWeight: 700, color: c.txt, margin: '0 0 2px' }}>Paystack</p>
          <p style={{ fontSize: 10, color: c.mut, margin: '0 0 10px' }}>Lagos · Fintech</p>
          <div style={{ textAlign: 'center', margin: '8px 0 12px' }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', border: `3px solid ${c.a}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 6px' }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: c.a }}>87</span>
            </div>
            <span style={{ fontSize: 9, color: c.mut }}>Overall Fit</span>
          </div>
          <div style={{ fontSize: 10, color: c.mut2, marginBottom: 10 }}>
          ...
          <div style={{ background: c.ad, border: `1px solid ${c.ab}`, borderRadius: 7,
            padding: '6px 10px', textAlign: 'center', fontSize: 10, fontWeight: 700, color: c.a }}>
            Write Email →
          </div>
        </div>
        ...
        <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 14,
          padding: 16, borderTop: `2px solid ${c.a}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
            <FileText size={12} color={c.a} />
            <span style={{ fontSize: 11, fontWeight: 700, color: c.a, letterSpacing: '0.05em', textTransform: 'uppercase' }}>CV Score</span>
          </div>
          <div style={{ textAlign: 'center', marginBottom: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', border: `3px solid ${c.a}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 6px' }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: c.a }}>78</span>
            </div>
            <span style={{ fontSize: 9, color: c.mut }}>out of 100</span>
          </div>
          <MiniBar label="Content"  pct="84%" color={c.a} />
          <MiniBar label="Keywords" pct="72%" color={c.a} />
          <MiniBar label="Format"   pct="65%" color="#34d399" />
          <div style={{ marginTop: 10, background: c.ad, border: `1px solid ${c.ab}`,
            borderRadius: 7, padding: '5px 8px', display: 'flex', alignItems: 'center', gap: 5 }}>
            <Wand2 size={9} color={c.a} />
            <span style={{ fontSize: 9, fontWeight: 700, color: c.a }}>3 improvements found</span>
          </div>
        </div>

        {/* Bottom row, match score + follow-up */}
        <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 12,
          ...
          <div>
            <p style={{ fontSize: 10, fontWeight: 700, color: c.txt, margin: 0 }}>Match Score</p>
            <p style={{ fontSize: 9, color: c.mut, margin: 0 }}>Flutterwave · PM role</p>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.2)',
          ...
          <p style={{ fontSize: 9, fontWeight: 700, color: '#34d399', margin: 0, display: 'flex', alignItems: 'center', gap: 3 }}><Check size={9} /> Safe, Verified</p>
          <p style={{ fontSize: 9, color: c.mut, margin: '2px 0 0' }}>0 red flags detected</p>
        </div>
      </div>
    </div>
  );
...
// ── Feature card ─────────────────────────────────────────────────────────────
function FeatureCard({ icon, title, desc, color = '#00D4C8', delay = 0 }) {
  const { colors: c } = useTheme();
  const ref = useReveal();
  return (
    <div ref={ref} className={`reveal reveal-delay-${delay}`}
      style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 14,
      ...
      <p style={{ fontSize: 15, fontWeight: 700, color: c.txt, margin: '0 0 6px' }}>{title}</p>
      <p style={{ fontSize: 13, color: c.mut, margin: 0, lineHeight: 1.6 }}>{desc}</p>
    </div>
  );
}

// ── Step ──────────────────────────────────────────────────────────────────────
function Step({ n, icon, title, desc, color = '#00D4C8' }) {
  const { colors: c } = useTheme();
  return (
    <div style={{ display: 'flex', gap: 16 }}>
      ...
      <div style={{ paddingTop: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          {icon}
          <p style={{ fontSize: 14, fontWeight: 700, color: c.txt, margin: 0 }}>{title}</p>
        </div>
        <p style={{ fontSize: 13, color: c.mut, margin: 0, lineHeight: 1.6 }}>{desc}</p>
      </div>
    </div>
...
function ScamDetectorWidget({ onGetStarted }) {
  ...
  const riskStyle = {
    ...
    Suspicious:  { color: c.a,       icon: <AlertTriangle size={18} color={c.a} />,   border: c.ab,                     bg: c.ad                       },
  ...
  return (
    <div>
      <textarea value={text} onChange={e => { setText(e.target.value); setResult(null); }}
        ...
        rows={5}
        style={{ width: '100%', background: 'rgba(0,0,0,0.4)', border: `1px solid ${c.ab}`,
          borderRadius: 10, padding: '12px 16px', color: c.txt, fontSize: 14,
          outline: 'none', resize: 'none', fontFamily: 'inherit', lineHeight: 1.6,
          boxSizing: 'border-box', transition: 'border-color 0.15s' }}
        onFocus={e => (e.target.style.borderColor = c.a)}
        onBlur={e  => (e.target.style.borderColor = c.ab)} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, flexWrap: 'wrap', gap: 8 }}>
        ...
        <button onClick={analyse} disabled={loading || text.length < 20}
          style={{ display: 'flex', alignItems: 'center', gap: 7,
            background: (loading || text.length < 20) ? c.ad : `linear-gradient(135deg, ${c.a}, #00B8AD)`,
            color: (loading || text.length < 20) ? c.a : c.bg,
            border: `1px solid ${c.ab}`, borderRadius: 9, padding: '10px 22px',
...
// ── Testimonial card ──────────────────────────────────────────────────────────
function Testimonial({ name, role, company, text, color = '#00D4C8' }) {
  const { colors: c } = useTheme();
  return (
    <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 14, padding: 24, flex: 1 }}>
      <div style={{ display: 'flex', gap: 2, marginBottom: 14 }}>
        {[1,2,3,4,5].map(i => <Star key={i} size={13} color={color} fill={color} />)}
      </div>
      <p style={{ fontSize: 14, color: c.txt, margin: '0 0 20px', lineHeight: 1.7, fontStyle: 'italic' }}>"{text}"</p>
      <div>
        <p style={{ fontSize: 13, fontWeight: 700, color: c.txt, margin: 0 }}>{name}</p>
        <p style={{ fontSize: 12, color: c.mut, margin: '2px 0 0' }}>{role} · {company}</p>
      </div>
    </div>
  );
}
...
function PricingTier({ name, price, period, features, cta, highlight, onGetStarted }) {
  const { colors: c } = useTheme();
  return (
    <div style={{ background: highlight ? `linear-gradient(135deg, ${c.ad}, rgba(0,212,200,0.06))` : c.card,
      border: `1px solid ${highlight ? c.ab : c.brd}`, borderRadius: 14, padding: 24, flex: 1 }}>
      {highlight && <div style={{ marginBottom: 10 }}><Pill>Most Popular</Pill></div>}
      <p style={{ fontSize: 13, fontWeight: 600, color: c.mut2, margin: '0 0 6px' }}>{name}</p>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 20 }}>
        <span style={{ fontSize: 32, fontWeight: 800, color: c.txt }}>{price}</span>
        {period && <span style={{ fontSize: 13, color: c.mut }}>{period}</span>}
      </div>
      <ul style={{ margin: '0 0 24px', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {features.map((f, i) => (
          <li key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 13, color: c.mut2 }}>
            <Check size={14} color="#34d399" style={{ flexShrink: 0, marginTop: 1 }} /> {f}
          </li>
        ))}
      </ul>
      <button onClick={onGetStarted}
        style={{ width: '100%', background: highlight ? `linear-gradient(135deg, ${c.a}, #00B8AD)` : 'transparent',
          color: highlight ? c.bg : c.a, border: `1.5px solid ${highlight ? 'transparent' : c.ab}`,
...
export default function LandingPage({ onGetStarted, onPricing }) {
  ...
  return (
    <div style={{ background: c.bg, color: c.txt, minHeight: '100vh', fontFamily: 'inherit' }}>

      {/* ── Navbar ───────────────────────────────────────────────────────────── */}
      <nav style={{
        position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50,
        background: scrolled || mobileMenu ? c.nav : 'transparent',
        backdropFilter: 'blur(16px)',
        borderBottom: scrolled ? `1px solid ${c.brd}` : '1px solid transparent',
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 20px', height: 64,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <div style={{ width: 30, height: 30, borderRadius: 8,
              ...
              <Zap size={15} color={c.bg} fill={c.bg} />
            </div>
            <span style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-0.5px', color: c.a,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {!isMobile && (
              <button onClick={onGetStarted}
                style={{ background: 'none', border: 'none', color: c.mut2, fontSize: 13, cursor: 'pointer' }}>
                Sign In
              </button>
            )}
            ...
            {isMobile && (
              <button onClick={() => setMobileMenu(m => !m)}
                style={{ background: 'none', border: `1px solid ${c.brd}`, borderRadius: 8,
                  color: c.txt, cursor: 'pointer', padding: '6px', display: 'flex',
      ...
      <section className="grid-bg" style={{ paddingTop: 'calc(120px + env(safe-area-inset-top, 0px))', paddingBottom: 80, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: -200, left: '50%', transform: 'translateX(-50%)',
          ...
          background: `radial-gradient(ellipse, ${c.ad} 0%, transparent 70%)`,
          pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px',
          ...
          <div ref={heroRef} className="reveal" style={{ maxWidth: 520 }}>
            ...
            <h1 style={{ fontSize: 'clamp(32px, 5vw, 54px)', fontWeight: 900,
              lineHeight: 1.1, letterSpacing: '-1.5px', color: c.txt, margin: '0 0 20px' }}>
              The AI platform that finds your next{' '}
              <span style={{ background: `linear-gradient(135deg, ${c.a}, #00D4C8)`,
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
                backgroundClip: 'text' }}>
                client, and your next job
              </span>
            </h1>
            <p style={{ fontSize: isMobile ? 16 : 18, color: c.mut, lineHeight: 1.7, margin: '0 0 32px', maxWidth: 440 }}>
              techcori gives companies sharp lead intelligence and job seekers a competitive edge,{' '}
              <span style={{ color: c.mut2 }}>powered by the same AI engine.</span>
            </p>
            <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 12 }}>
              <GradBtn onClick={onGetStarted} style={isMobile ? { width: '100%' } : {}}>
                I am a Company →
              </GradBtn>
              <OutlineBtn onClick={onGetStarted} color={c.a} style={isMobile ? { width: '100%', textAlign: 'center' } : {}}>
                I am a Job Seeker →
              </OutlineBtn>
            </div>
      ...
      <div ref={statsRef} id="features"
        style={{ background: c.card, borderTop: `1px solid ${c.brd}`, borderBottom: `1px solid ${c.brd}`,
          padding: '40px 24px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', textAlign: 'center' }}>
          <p style={{ fontSize: 13, color: c.mut, margin: '0 0 28px', letterSpacing: '0.04em', textTransform: 'uppercase', fontWeight: 600 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32, maxWidth: 600, margin: '0 auto' }}>
            {[
              ...
              <div key={label}>
                <div style={{ fontSize: 'clamp(28px,4vw,42px)', fontWeight: 900, color: c.a, letterSpacing: '-1px' }}>
      ...
      <section style={{ padding: '96px 24px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <div ref={compRef} className="reveal" style={{ marginBottom: 56, maxWidth: 560 }}>
            <Pill color={c.a}>For Companies</Pill>
            <h2 style={{ fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 800, letterSpacing: '-1px',
              color: c.txt, margin: '14px 0 14px', lineHeight: 1.15 }}>
              Find, Research, and Close
            </h2>
            <p style={{ fontSize: 16, color: c.mut, lineHeight: 1.7, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
            ...
            <FeatureCard delay={3} color={c.a}
              icon={<Mail size={18} color={c.a} />}
          ...
          <div ref={compMockupRef} className="reveal" style={{ marginTop: 56 }}>
            <div className="animate-float-slow" style={{ background: c.card, border: `1px solid ${c.brd}`,
              borderRadius: 18, padding: 24, maxWidth: 680,
              boxShadow: `0 0 60px ${c.ad}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#f87171' }} />
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: c.a }} />
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#34d399' }} />
                <span style={{ fontSize: 11, color: c.mut, marginLeft: 8 }}>techcori · Company Intelligence</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16 }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: c.mut, margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Company Profile</p>
                  <p style={{ fontSize: 16, fontWeight: 800, color: c.txt, margin: '0 0 4px' }}>Flutterwave</p>
                  <p style={{ fontSize: 12, color: c.mut, margin: '0 0 16px' }}>Lagos · Fintech · 500-1000 employees</p>
                  {[['Role Fit', 91, c.a], ['Culture Fit', 84, c.a], ['Hiring Activity', 78, '#34d399'], ['Overall', 87, c.a]].map(([l, v, col]) => (
                    <MiniBar key={l} label={l} pct={`${v}%`} color={col} />
                  ))}
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: c.mut, margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Hiring Signals</p>
                  {['Expanding product team (Q4 2025)', 'Senior engineer roles posted', '4 open positions on careers page', 'Recent Series C announcement'].map((s, i) => (
                    <div key={i} style={{ display: 'flex', gap: 7, alignItems: 'flex-start', marginBottom: 8 }}>
                      ...
                      <span style={{ fontSize: 12, color: c.mut2, lineHeight: 1.5 }}>{s}</span>
                    </div>
                  ))}
                  <div style={{ marginTop: 16, background: c.ad, border: `1px solid ${c.ab}`,
                    borderRadius: 9, padding: '10px 14px', textAlign: 'center' }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: c.a, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><Mail size={13} /> Write Outreach Email</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section style={{ padding: '96px 24px', background: 'rgba(0,212,200,0.02)' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <div ref={seekerRef} className="reveal" style={{ marginBottom: 56, maxWidth: 560 }}>
            <Pill color={c.a}>For Job Seekers</Pill>
            <h2 style={{ fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 800, letterSpacing: '-1px',
              color: c.txt, margin: '14px 0 14px', lineHeight: 1.15 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
            <FeatureCard delay={1} color={c.a}
              icon={<FileText size={18} color={c.a} />}
              title="CV Optimiser"
            ...
            <FeatureCard delay={2} color={c.a}
              icon={<Building2 size={18} color={c.a} />}
          ...
          <div ref={seekerMockupRef} className="reveal" style={{ marginTop: 56 }}>
            <div className="animate-float-slow" style={{ background: c.card, border: `1px solid ${c.brd}`,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                ...
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: c.a }} />
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#34d399' }} />
                <span style={{ fontSize: 11, color: c.mut, marginLeft: 8 }}>techcori · CV Optimiser</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'auto 1fr', gap: 24, alignItems: 'start' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ width: 84, height: 84, borderRadius: '50%',
                    border: `5px solid ${c.a}`, display: 'flex', alignItems: 'center',
                    justifyContent: 'center', margin: '0 auto 8px',
                    boxShadow: `0 0 24px ${c.ad}` }}>
                    <span style={{ fontSize: 28, fontWeight: 900, color: c.a }}>78</span>
                  </div>
                  <p style={{ fontSize: 12, color: c.mut, margin: 0 }}>CV Score</p>
                  <p style={{ fontSize: 11, color: '#264040', margin: '4px 0 0' }}>out of 100</p>
                </div>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: c.mut, margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Score Breakdown</p>
                  <MiniBar label="Content (40pts)"  pct="84%" color={c.a} />
                  <MiniBar label="Format (25pts)"   pct="72%" color={c.a} />
                  ...
                  <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {['Add quantified achievements to Experience bullets', 'Include "Python" and "SQL" in your Skills section'].map((s, i) => (
                      <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', background: c.ad,
                        border: `1px solid ${c.ab}`, borderRadius: 7, padding: '8px 10px' }}>
                        <Wand2 size={11} color={c.a} style={{ flexShrink: 0, marginTop: 1 }} />
                        <span style={{ fontSize: 11, color: c.mut2, lineHeight: 1.5 }}>{s}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section id="how" style={{ padding: '96px 24px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div ref={howRef} className="reveal" style={{ textAlign: 'center', marginBottom: 48 }}>
            <h2 style={{ fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 800, letterSpacing: '-1px',
              color: c.txt, margin: '0 0 16px' }}>
              How It Works
            </h2>
            <p style={{ fontSize: 16, color: c.mut, margin: '0 0 28px' }}>
              From zero to outreach in under 5 minutes.
            </p>
            <div style={{ display: 'inline-flex', background: c.card, border: `1px solid ${c.brd}`,
              borderRadius: 12, padding: 4, gap: 4 }}>
              {[['company', 'For Companies', c.a], ['jobseeker', 'For Job Seekers', c.a]].map(([id, label, color]) => (
                <button key={id} onClick={() => setHowTab(id)}
                  style={{ padding: '8px 20px', borderRadius: 9, border: 'none', fontSize: 13, fontWeight: 600,
                    ...
                    color: howTab === id ? color : c.mut, cursor: 'pointer', transition: 'all 0.2s' }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={{ background: c.card, border: `1px solid ${c.brd}`, borderRadius: 18, padding: 40 }}>
            {howTab === 'company' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <Step n={1} color={c.a}
                  icon={<Search size={15} color={c.a} />}
                ...
                <Step n={2} color={c.a}
                  icon={<BarChart2 size={15} color={c.a} />}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <Step n={1} color={c.a}
                  icon={<FileText size={15} color={c.a} />}
                ...
                <Step n={2} color={c.a}
                  icon={<Building2 size={15} color={c.a} />}
      ...
      <section ref={scamRef} className="reveal"
        style={{ padding: '80px 24px', background: `linear-gradient(135deg, ${c.ad}, rgba(0,212,200,0.04))`,
          borderTop: `1px solid ${c.ab}`, borderBottom: `1px solid ${c.ab}` }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: c.ad,
              border: `1px solid ${c.ab}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Shield size={20} color={c.a} />
            </div>
            <Pill color={c.a}>Free Tool · No Sign-up</Pill>
          </div>
          <h2 style={{ fontSize: 'clamp(24px,3.5vw,38px)', fontWeight: 800, letterSpacing: '-1px',
            color: c.txt, margin: '0 0 12px', lineHeight: 1.2 }}>
            Is that job real?{' '}
            <span style={{ background: `linear-gradient(135deg, ${c.a}, #00D4C8)`,
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
              backgroundClip: 'text' }}>
          ...
          <p style={{ fontSize: 15, color: c.mut, margin: '0 0 28px', lineHeight: 1.7 }}>
      ...
      <section id="pricing" style={{ padding: '96px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div ref={pricingRef} className="reveal" style={{ textAlign: 'center', marginBottom: 56 }}>
            <h2 style={{ fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 800, letterSpacing: '-1px',
              color: c.txt, margin: '0 0 12px' }}>
              Simple Pricing
            </h2>
            <p style={{ fontSize: 16, color: c.mut, margin: 0 }}>Start free. Upgrade when you're ready.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32 }}>
            ...
            {/* Job Seeker */}
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: c.a, textTransform: 'uppercase',
      ...
      {/* ── Testimonials ─────────────────────────────────────────────────────── */}
      <section style={{ padding: '96px 24px', background: c.card, borderTop: `1px solid ${c.brd}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div ref={testiRef} className="reveal" style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <p style={{ fontSize: 15, color: c.mut, margin: 0 }}>Early users across Lagos, Abuja, and Port Harcourt</p>
          </div>
      ...
      <section className="grid-bg" style={{ padding: '100px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(28px,4vw,48px)', fontWeight: 900, letterSpacing: '-1.5px',
            color: c.txt, margin: '0 0 16px', lineHeight: 1.1 }}>
            work smart,{' '}
            <span style={{ background: `linear-gradient(135deg, ${c.a}, #00D4C8)`,
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
              backgroundClip: 'text' }}>
          ...
          <p style={{ fontSize: 17, color: c.mut, margin: '0 0 36px', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <OutlineBtn onClick={() => scrollTo('features')} color={c.mut}>
              See how it works
            </OutlineBtn>
          </div>
        </div>
      ...
      {/* ── Footer ───────────────────────────────────────────────────────────── */}
      <footer style={{ background: c.footer, borderTop: `1px solid ${c.brd}`, padding: '48px 24px 32px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'auto 1fr auto', gap: isMobile ? 24 : 40, alignItems: 'start', marginBottom: 40 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8,
                  background: `linear-gradient(135deg, #00D4C8, #00B8AD)`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Zap size={13} color={c.bg} fill={c.bg} />
                </div>
                <span style={{ fontSize: 16, fontWeight: 800, color: c.a, fontFamily: "'Clash Display', sans-serif" }}>techcori</span>
              </div>
              <p style={{ fontSize: 12, color: c.mut, margin: 0, lineHeight: 1.6, maxWidth: 200 }}>
            ...
            <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap' }}>
              {[
                ...
                <div key={heading}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: c.mut, margin: '0 0 12px',
                    textTransform: 'uppercase', letterSpacing: '0.06em' }}>{heading}</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {links.map(l => (
                      <span key={l} style={{ fontSize: 13, color: c.mut, cursor: 'pointer',
                        transition: 'color 0.15s' }}
                        onMouseEnter={e => (e.currentTarget.style.color = c.mut2)}
                        onMouseLeave={e => (e.currentTarget.style.color = c.mut)}>
                        {l}
                      </span>
                    ))}
